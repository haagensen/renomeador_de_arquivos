...
def titlecase_arquivo(path, aplicar_sobre="n", alterar=False):
    total = 0
    saida = tuple()
    ...
    for pathname, _, filenames in os.walk(path):
        for f in filenames:
            ...
            if aplicar_sobre == "c":
                ...
                pos = f.rfind(".")
                if aplicar_sobre == "n":
                    arq = f[0:pos].title() + f[pos:]
                else:
                    arq = f[0:pos] + f[pos:].title()

            if f != arq:
                if alterar:
                    os.rename(os.path.join(pathname, f),
                              os.path.join(pathname, arq))
                tupla = (os.path.join(pathname, f), 
                        os.path.join(pathname, arq))
                saida += tupla,
...
if __name__ == "__main__":
    ...
    print('Renomeador de Arquivos - nº2')
    ...
    total, arquivos = titlecase_arquivo(path, aplicarSobre, alterar)
