...
def lowercase_arquivos(path, aplicarsobre="n", alterar=False):
    total = 0
    saida = tuple()
    ...
    for pathname, _, filenames in os.walk(path):
        for f in filenames:
            ...
            if aplicarsobre == "c":
                ...
                pos = f.rfind(".")
                if aplicarsobre == "n":
                    arq = f[0:pos].lower() + f[pos:]
                else:
                    arq = f[0:pos] + f[pos:].lower()
...
if __name__ == "__main__":
    ...
    print('Renomeador de Arquivos - nº4')
    ...
    total, arquivos = lowercase_arquivos(path, aplicarSobre, alterar)
